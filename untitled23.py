...
# -*- coding: utf-8 -*-
"""
Created on Sun Oct  3 22:42:41 2021
...
# file2 = '/Volumes/GoogleDrive/マイドライブ/lab/solar_burst/Nancay/af_sgepss_analysis_data/test/60dB_70dB_gain_analysis.csv'
# file2 = '/Volumes/GoogleDrive/マイドライブ/lab/solar_burst/Nancay/af_sgepss_analysis_data/test/ver2_30dB_40dB_gain_analysis.csv'
file2 = '/Volumes/GoogleDrive/マイドライブ/lab/solar_burst/Nancay/af_sgepss_analysis_data/test/RR_LL_30dB_40dB_gain_analysis.csv'
...
# check_one_date = 20131229


Parent_directory = '/Volumes/GoogleDrive/マイドライブ/lab'
Parent_lab = len(Parent_directory.split('/')) - 1
# file_final = '/Volumes/GoogleDrive/マイドライブ/lab/solar_burst/Nancay/af_sgepss_analysis_data/RR_LL_gain/new_ver2_gain_analysis_20070101_20201231.csv'
...
BG_obs_time_test = BG_obs_time[(BG_obs_time>datetime.datetime(int(str(start_day)[0:4]),int(str(start_day)[4:6]),int(str(start_day)[6:8])))&(BG_obs_time<datetime.datetime(int(str(end_day)[0:4]),int(str(end_day)[4:6]),int(str(end_day)[6:8])) + datetime.timedelta(days=1))]
BG_list_start = np.where(BG_obs_time == BG_obs_time_test[0])[0][0]
BG_list_end = np.where(BG_obs_time == BG_obs_time_test[-1])[0][0]
BG_obs_time = BG_obs_time[BG_list_start: BG_list_end+1]
decibel_list = decibel_list[BG_list_start: BG_list_end+1,:]
...
for file in file_list:
    ...
    for i in range(len(csv_input)):
        ...
        if (obs_time_event >datetime.datetime(int(str(start_day)[0:4]),int(str(start_day)[4:6]),int(str(start_day)[6:8])))&(obs_time_event<datetime.datetime(int(str(end_day)[0:4]),int(str(end_day)[4:6]),int(str(end_day)[6:8])) + datetime.timedelta(days=1)):
            # Frequency_list = csv_input['Frequency'][i]
            Frequency = np.array([float(k) for k in csv_input['Frequency'][i][1:-1].replace('\n', '').split(' ') if k != ''])
            gain_r = np.array([float(k) for k in csv_input['Right-gain'][i][1:-1].replace('\n', '').split(' ') if k != ''])
            Trx_r = np.array([float(k) for k in csv_input['Right-Trx'][i][1:-1].replace('\n', '').split(' ') if k != ''])
            gain_l = np.array([float(k) for k in csv_input['Left-gain'][i][1:-1].replace('\n', '').split(' ') if k != ''])
            Trx_l = np.array([float(k) for k in csv_input['Left-Trx'][i][1:-1].replace('\n', '').split(' ') if k != ''])
            ...
            gain_40_list_r.append(gain_r[0])
            gain_37_5_list_r.append(gain_r[1])
            gain_35_list_r.append(gain_r[2])
            gain_32_5_list_r.append(gain_r[3])
            gain_30_list_r.append(gain_r[4])
            # hot_40_list.append(hot_dB[0])
            # cold_40_list.append(cold_dB[0])
            Trx_40_r.append(Trx_r[0])
            Trx_37_5_r.append(Trx_r[1])
            Trx_35_r.append(Trx_r[2])
            Trx_32_5_r.append(Trx_r[3])
            Trx_30_r.append(Trx_r[4])
    
            gain_40_list_l.append(gain_l[0])
            gain_37_5_list_l.append(gain_l[1])
            gain_35_list_l.append(gain_l[2])
            gain_32_5_list_l.append(gain_l[3])
            gain_30_list_l.append(gain_l[4])
            # hot_40_list.append(hot_dB[0])
            # cold_40_list.append(cold_dB[0])
            Trx_40_l.append(Trx_l[0])
            Trx_37_5_l.append(Trx_l[1])
            Trx_35_l.append(Trx_l[2])
            Trx_32_5_l.append(Trx_l[3])
            Trx_30_l.append(Trx_l[4])

# print ('Done')
# plt.plot(gain_obs_time, Trx_40_r)
# plt.ylim(1000,3000)
# plt.show()
# plt.close()

gain_obs_time = np.array(gain_obs_time)
gain_40_list_r = np.array(gain_40_list_r)
gain_37_5_list_r = np.array(gain_37_5_list_r)
gain_35_list_r = np.array(gain_35_list_r)
gain_32_5_list_r = np.array(gain_32_5_list_r)
gain_30_list_r = np.array(gain_30_list_r)
gain_40_list_l = np.array(gain_40_list_l)
gain_37_5_list_l = np.array(gain_37_5_list_l)
gain_35_list_l = np.array(gain_35_list_l)
gain_32_5_list_l = np.array(gain_32_5_list_l)
gain_30_list_l = np.array(gain_30_list_l)

Trx_40_r = np.array(Trx_40_r)
Trx_37_5_r = np.array(Trx_37_5_r)
Trx_35_r = np.array(Trx_35_r)
Trx_32_5_r = np.array(Trx_32_5_r)
Trx_30_r = np.array(Trx_30_r)
Trx_40_l = np.array(Trx_40_l)
Trx_37_5_l = np.array(Trx_37_5_l)
Trx_35_l = np.array(Trx_35_l)
Trx_32_5_l = np.array(Trx_32_5_l)
Trx_30_l = np.array(Trx_30_l)
# hot_40_list = np.array(hot_40_list_r)
# cold_40_list = np.array(cold_40_list_r)
...
for i in range(len(BG_obs_time)):
    ...
    if abs(gain_obs_time[gain_index] - check_date) <= datetime.timedelta(seconds=60*90):
        cali_dB_40_list_r.append(dB_40_list[i]+(np.log10(np.nanmedian(gain_40_list_r)/gain_40_list_r[gain_index]) * 10))
        cali_dB_37_5_list_r.append(dB_37_5_list[i]+(np.log10(np.nanmedian(gain_37_5_list_r)/gain_37_5_list_r[gain_index]) * 10))
        cali_dB_35_list_r.append(dB_35_list[i]+(np.log10(np.nanmedian(gain_35_list_r)/gain_35_list_r[gain_index]) * 10))
        cali_dB_32_5_list_r.append(dB_32_5_list[i]+(np.log10(np.nanmedian(gain_32_5_list_r)/gain_32_5_list_r[gain_index]) * 10))
        cali_dB_30_list_r.append(dB_30_list[i]+(np.log10(np.nanmedian(gain_30_list_r)/gain_30_list_r[gain_index]) * 10))
    else:
        cali_dB_40_list_r.append(np.nan)
        cali_dB_37_5_list_r.append(np.nan)
        cali_dB_35_list_r.append(np.nan)
        cali_dB_32_5_list_r.append(np.nan)
        cali_dB_30_list_r.append(np.nan)
cali_dB_40_list = np.array(cali_dB_40_list_r)
cali_dB_37_5_list = np.array(cali_dB_37_5_list_r)
cali_dB_35_list = np.array(cali_dB_35_list_r)
cali_dB_32_5_list = np.array(cali_dB_32_5_list_r)
cali_dB_30_list = np.array(cali_dB_30_list_r)
...
# # date_in = [check_one_date,check_one_date]
# start_day,end_day=date_in
# sdate=pd.to_datetime(start_day,format='%Y%m%d')
# edate=pd.to_datetime(end_day,format='%Y%m%d')
...
#                 if i == 0:
#                     ax1.plot(gain_obs_time[idxes_gain], gain_40_list_r[idxes_gain], '.--', label = '40MHz')
#                     ax2.plot(gain_obs_time[idxes_gain], gain_37_5_list_r[idxes_gain], '.--', label = '37.5MHz')
#                     ax3.plot(gain_obs_time[idxes_gain], gain_35_list_r[idxes_gain], '.--', label = '35MHz')
#                     ax4.plot(gain_obs_time[idxes_gain], gain_32_5_list_r[idxes_gain], '.--', label = '32.5MHz')
#                     ax5.plot(gain_obs_time[idxes_gain], gain_30_list_r[idxes_gain], '.--', label = '30MHz')

#                 else:
#                     ax1.plot(gain_obs_time[idxes_gain], gain_40_list_r[idxes_gain], '.--')
#                     ax2.plot(gain_obs_time[idxes_gain], gain_37_5_list_r[idxes_gain], '.--')
#                     ax3.plot(gain_obs_time[idxes_gain], gain_35_list_r[idxes_gain], '.--')
#                     ax4.plot(gain_obs_time[idxes_gain], gain_32_5_list_r[idxes_gain], '.--')
#                     ax5.plot(gain_obs_time[idxes_gain], gain_30_list_r[idxes_gain], '.--')
...
#             ymax_gain = np.max([np.nanmax(gain_40_list_r[full_idxes_gain]), np.nanmax(gain_37_5_list_r[full_idxes_gain]), np.nanmax(gain_35_list_r[full_idxes_gain]), np.nanmax(gain_32_5_list_r[full_idxes_gain]), np.nanmax(gain_30_list_r[full_idxes_gain])])
#             ymin_gain = np.min([np.nanmin(gain_40_list_r[full_idxes_gain]), np.nanmin(gain_37_5_list_r[full_idxes_gain]), np.nanmin(gain_35_list_r[full_idxes_gain]), np.nanmin(gain_32_5_list_r[full_idxes_gain]), np.nanmin(gain_30_list_r[full_idxes_gain])])
