...
def threshold_array(diff_move_db_RR_sep, diff_move_db_LL_sep, freq_start_idx, freq_end_idx, sigma_value, Frequency, threshold_frequency, duration, db_threshold_percentail,db_range, Status_sep):
    quartile_db_r = []
    mean_r_list = []
    mean_l_list = []
    stdev_sub = []
    ...
    diff_move_db_LL_sep = np.delete(diff_move_db_LL_sep, cali_place, 1)


    for i in range(diff_move_db_RR_sep.shape[0]):
    #    for i in range(0, 357, 1):
        quartile_db_25_1_r = np.percentile(diff_move_db_RR_sep[i], db_threshold_percentail)
        quartile_db_25_2_r = np.percentile(diff_move_db_RR_sep[i], db_threshold_percentail-db_range)
        quartile_db_each_r = []
        for k in range(diff_move_db_RR_sep[i].shape[0]):
            if diff_move_db_RR_sep[i][k] <= quartile_db_25_1_r:
                if diff_move_db_RR_sep[i][k] >= quartile_db_25_2_r:
        ...
        # print (math.log10(m_r) * 10)
        #                quartile_db_each.append(math.log10(diff_power) * 10)
    for i in range(diff_move_db_LL_sep.shape[0]):
    #    for i in range(0, 357, 1):
        quartile_db_25_1_l = np.percentile(diff_move_db_LL_sep[i], db_threshold_percentail)
        quartile_db_25_2_l = np.percentile(diff_move_db_LL_sep[i], db_threshold_percentail-db_range)
        quartile_db_each_l = []
        for k in range(diff_move_db_LL_sep[i].shape[0]):
            if diff_move_db_LL_sep[i][k] <= quartile_db_25_1_l:
                if diff_move_db_LL_sep[i][k] >= quartile_db_25_2_l:
                    diff_power_quartile_l = (10 ** ((diff_move_db_LL_sep[i][k])/10))
    ...
    return mean_r_list, mean_l_list
...
db_check_mean_list_l = []
db_check_mean_list_r = []
...
# years = ['2013']
with open(Parent_directory+ '/solar_burst/Nancay/af_sgepss_analysis_data/RR_LL_antenna/RR_LL_all_freq_under25_'+str(date_in[0])+'_'+str(date_in[1])+'.csv', 'w') as f:
    w = csv.DictWriter(f, fieldnames=["obs_time", "freq_start", "freq_end", "BG_RR", "BG_LL"])
    w.writeheader()
    ...
    while DATE <= edate:
        ...
        #     w.writeheader()
            ...
            # for sep_place in sep_places:
                ...
                mean_r_list, mean_l_list = threshold_array(diff_move_db_RR_sep, diff_move_db_LL_sep, freq_start_idx, freq_end_idx, sigma_value, Frequency, threshold_frequency, duration, db_threshold_percentail, db_range, Status_sep)
                # plot_array_threshold(arr_threshold, x_lims, Frequency, date_OBs, freq_start_idx, freq_end_idx, db_setting, min_db, quartile_db_l)
                mean_list_r.append(mean_r_list)
                mean_list_l.append(mean_l_list)
                ...
                w.writerow({'obs_time':obs_time, 'freq_start':Frequency_start, 'freq_end':Frequency_end, 'BG_RR':mean_r_list, 'BG_LL':mean_l_list})
            db_check_mean_list_r.append(np.array(mean_list_r))
            db_check_mean_list_l.append(np.array(mean_list_l))

        
            # if len(mean_list_l) == 1:
            #     mean_list = []
            #     time_list_oneday = []
            #     for t in range(2):
