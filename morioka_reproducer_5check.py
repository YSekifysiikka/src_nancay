...
def plot_green(around, ax, dB_BG, check_freq):
    ...
    if plot_type == 'rgb':
        ax.scatter(np.arange(0,time_band+time_co,1)[maxid[0]], around[maxid[0]], color = "blue")
    maxid_list = []
    for maxidx in maxid:
...
# def radio_plot(diff_db_plot_sep, x_lims, Frequency_start, Frequency_end, Time_start, Time_end, date_OBs, burst_type, detected_files, save_dir, width, Status_sep, stime):
#     dB_list = []
#     burst_time_list = []
...
#     gs = gridspec.GridSpec(191, 1)
#     ax = plt.subplot(gs[0:40, :])
#     ax4 = plt.subplot(gs[50:70, :])
#     ax2 = plt.subplot(gs[95:110, :])#40MHz
#     ax3 = plt.subplot(gs[115:130, :])#37.5MHz
#     ax1 = plt.subplot(gs[135:150, :])#35MHz
#     ax5 = plt.subplot(gs[155:170, :])#32.5MHz
#     ax6 = plt.subplot(gs[175:190, :])#30MHz
...
#     around_40 = np.nanmedian(diff_db_plot_sep[int(db_40-((check_data-1)/2)):int(db_40+((check_data-1)/2)+1)], axis =0)
#     select_array_empty[int(db_40-((check_data-1)/2)):int(db_40+((check_data-1)/2)+1)] = diff_db_plot_sep[int(db_40-((check_data-1)/2)):int(db_40+((check_data-1)/2)+1)]
...
#     around_35 = np.nanmedian(diff_db_plot_sep[int(db_35-((check_data-1)/2)):int(db_35+((check_data-1)/2)+1)], axis =0)
#     if check_data > 1:
#         select_array_empty[int(db_35-((check_data-1)/2)):int(db_35+((check_data-1)/2)+1)] = diff_db_plot_sep[int(db_35-((check_data-1)/2)):int(db_35+((check_data-1)/2)+1)]
#     else:
#         select_array_empty[int(db_35)] = diff_db_plot_sep[int(db_35)]
...
#     around_30 = np.nanmedian(diff_db_plot_sep[int(db_30-((check_data-1)/2)):int(db_30+((check_data-1)/2)+1)], axis =0)
#     if check_data > 1:
#         select_array_empty[int(db_30-((check_data-1)/2)):int(db_30+((check_data-1)/2)+1)] = diff_db_plot_sep[int(db_30-((check_data-1)/2)):int(db_30+((check_data-1)/2)+1)]
#     else:
#         select_array_empty[int(db_30)] = diff_db_plot_sep[int(db_30)]
...
#     # around_50 = np.log10(((np.sum(10 ** ((diff_db_plot_sep[int(db_50-((check_data-1)/2)):int(db_50+((check_data-1)/2)+1)])/10), axis =0))/check_data)) * 10
#     around_37_5 = np.nanmedian(diff_db_plot_sep[int(db_37_5-((check_data-1)/2)):int(db_37_5+((check_data-1)/2)+1)], axis =0)
#     if check_data > 1:
#         select_array_empty[int(db_37_5-((check_data-1)/2)):int(db_37_5+((check_data-1)/2)+1)] = diff_db_plot_sep[int(db_37_5-((check_data-1)/2)):int(db_37_5+((check_data-1)/2)+1)]
#     else:
#         select_array_empty[int(db_37_5)] = diff_db_plot_sep[int(db_37_5)]
#     # around_over0 = around_50[around_50>0]
#     # under_dBsetting_val = np.percentile(around_over0, 25)
#     # under_dB_threshold_around = around_over0[around_over0<=under_dBsetting_val]
#     # dB_BG_50 = np.mean(under_dB_threshold_around) + 2*np.std(under_dB_threshold_around)

#     under_dBsetting_val = np.nanpercentile(around_37_5, dB_threshold)
#     under_dB_threshold_around = around_37_5[~np.isnan(around_37_5)][(around_37_5[~np.isnan(around_37_5)]<=under_dBsetting_val)]
#     dB_BG_37_5 = np.mean(under_dB_threshold_around) + 2*np.std(under_dB_threshold_around)


#     db_32_5 = np.where(Frequency == func_small.getNearestValue(Frequency,32.5))[0][0]
#     # around_30 = np.log10(((np.sum(10 ** ((diff_db_plot_sep[int(db_30-((check_data-1)/2)):int(db_30+((check_data-1)/2)+1)])/10), axis =0))/check_data)) * 10
#     around_32_5 = np.nanmedian(diff_db_plot_sep[int(db_32_5-((check_data-1)/2)):int(db_32_5+((check_data-1)/2)+1)], axis =0)
#     if check_data > 1:
#         select_array_empty[int(db_32_5-((check_data-1)/2)):int(db_32_5+((check_data-1)/2)+1)] = diff_db_plot_sep[int(db_32_5-((check_data-1)/2)):int(db_32_5+((check_data-1)/2)+1)]
#     else:
#         select_array_empty[int(db_32_5)] = diff_db_plot_sep[int(db_32_5)]
#     # around_over0 = around_30[around_30>0]
#     # under_dBsetting_val = np.percentile(around_over0, 25)
#     # under_dB_threshold_around = around_over0[around_over0<=under_dBsetting_val]
#     # dB_BG__dB_threshold = np.mean(under_dB_threshold_around) + 2*np.std(under_dB_threshold_around)
#     under_dBsetting_val = np.nanpercentile(around_32_5, dB_threshold)
#     under_dB_threshold_around = around_32_5[~np.isnan(around_32_5)][(around_32_5[~np.isnan(around_32_5)]<=under_dBsetting_val)]
#     dB_BG_32_5 = np.mean(under_dB_threshold_around) + 2*np.std(under_dB_threshold_around)
# ########################################

#     if check_data > 1:
#         ax4.imshow(select_array_empty[int(db_40-((check_data-1)/2)):int(db_30+((check_data-1)/2)+1)], extent = [x_lims[0], x_lims[1],  Frequency[int(db_30+((check_data-1)/2)+1)], Frequency[int(db_40-((check_data-1)/2))]], 
#                   aspect='auto',cmap='jet',vmin=5,vmax=30)
#     else:
#         ax4.imshow(select_array_empty[int(db_40):int(db_30)+1], extent = [x_lims[0], x_lims[1],  Frequency[int(db_30)], Frequency[int(db_40)]], 
#                   aspect='auto',cmap='jet',vmin=5,vmax=30)
#     ax4.xaxis_date()
...
#     # ax1.plot(np.arange(0,time_band+time_co,1), around_35, '.-', linewidth=0.5, markersize=1, label = '35MHz')
#     # # ax1.axhline(dB_line, ls = "-.", color = "magenta", label = str(dB_line)+'dB')
#     # ax1.axhline(dB_BG_35, ls = "-.", color = "r", label = str(round(dB_BG_35,1))+'dB')
#     # if plot_type == 'rgb':
#     #     ax1.scatter(np.arange(0,time_band+time_co,1)[maxid[0]], around_35[maxid[0]], color = "blue")
#     # maxid3_list = []
...
#     #                 if plot_type == 'rgb':
#     #                     ax1.scatter(np.arange(0,time_band+time_co,1)[maxidx], around_35[maxidx], color = "g")
...
#     # ax6.plot(np.arange(0,time_band+time_co,1), around_30, '.-', linewidth=0.5, markersize=1, label = '30MHz')
#     # # ax6.axhline(dB_line, ls = "-.", color = "magenta", label = str(dB_line)+'dB')
#     # ax6.axhline(dB_BG_30, ls = "-.", color = "r", label = str(round(dB_BG_30,1))+'dB')
#     # if plot_type == 'rgb':
#     #     ax6.scatter(np.arange(0,time_band+time_co,1)[maxid[0]], around_30[maxid[0]], color = "blue")
#     # maxid1_list = []
#     # for maxidx in maxid[0]:
#     #     if threshold_time % 2 == 1:
#     #         if (maxidx - int(threshold_time/2) >= 0) & (maxidx + int(threshold_time/2) < time_band + time_co):
...
#     #                 if plot_type == 'rgb':
#     #                     ax6.scatter(np.arange(0,time_band+time_co,1)[maxidx], around_30[maxidx], color = "g")
...
#                                             dB_list.append(around_40[maxid2])
#                                             bstime = epoch[stime+np.arange(0,time_band+time_co,1)[maxid3]+1]
#                                             burst_time_list.append(datetime.datetime(bstime[0], bstime[1], bstime[2], bstime[3], bstime[4], bstime[5], bstime[6]))
...
#     #                labelright=False,
#     #                labeltop=False)
#     # ax5.set_xlim(0,time_band+time_co-1)
#     # ax5.legend(fontsize = 12)
#     # ax5.tick_params(labelbottom=False,
#     #                labelright=False,
#     #                labeltop=False)
#     #                 # ax6.scatter(np.arange(0,time_band+time_co,1)[maxidx], around_30[maxidx], color = "blue")
#     # # ax2.scatter(np.arange(0,time_band+time_co,1)[maxid[0]], around_40[maxid[0]])
#     # # ax6.scatter(np.arange(0,time_band+time_co,1)[maxid[0]], around_30[maxid[0]])
#     # ax6.set_xlim(0,time_band+time_co-1)
...
#     plt.show()
#     return dB_list, burst_time_list, dB_BG_40
...
v_list = [19, 35]
burst_type = 'fao'
save_dir = 'LL_RR_freq_check_around'+str(check_data)+'_width'+str(width)+'_mediancos_dBthreshold'+str(dB_threshold)+'_threshold_time'+str(threshold_time)+'_skiprate'+str(skip_rate)
plot_type = 'rgb'
...
date_in=[20090813,20090927]
with open(Parent_directory+ '/solar_burst/Nancay/af_sgepss_analysis_data/morioka_analysis/'+str(date_in[0])+'_'+str(date_in[1])+'.csv', 'w') as f:
    w = csv.DictWriter(f, fieldnames=["Time_list", "intensity_list", "BG_40[dB]"])
    w.writeheader()
        ...
        if len(glob.glob('/Volumes/GoogleDrive/マイドライブ/lab/solar_burst/Nancay/data/'+year+'/'+mm+'/*'+ date +'*cdf')) >0:
            ...
            for t in range (math.floor(((diff_move_RR_db.shape[1]-time_co)/time_band) + 1)):
                ...
                ########################################
                                                ...
                                                for maxid4 in maxid4_list[(maxid4_list >= maxid2) & (maxid4_list <= maxid2 + 1)]:
                                                    ...
                                                    if len(maxid5_list[(maxid5_list >= maxid3) & (maxid5_list <= maxid3 + 1)]) >= 0:
                                                        for maxid5 in maxid5_list[(maxid5_list >= maxid3) & (maxid5_list <= maxid3 + 1)]:
                                                            ...
                                                            dB_list.append(burst_intensity)
                                                            bstime = epoch[stime+np.arange(0,time_band+time_co,1)[maxid3]+1]
                                                            burst_time_list.append(datetime.datetime(bstime[0], bstime[1], bstime[2], bstime[3], bstime[4], bstime[5], bstime[6]))
                    plot_arange([ax1, ax2, ax3, ax5, ax6])
                    ...
                    u, indices = np.unique(burst_time_list, return_index=True)
                    burst_time_list_final.extend(u.tolist())
                    dB_list_final.extend(np.array(dB_list)[indices].tolist())
                    dB_BG_40_list.append(dB_BG_40)
                    if len(u.tolist())>0:
                        for i in range(len(u.tolist())):
                            w.writerow({'Time_list': u.tolist()[i], 'intensity_list': np.array(dB_list)[indices].tolist()[i], 'BG_40[dB]': dB_BG_40})
                    # sys.exit()
                # except:
