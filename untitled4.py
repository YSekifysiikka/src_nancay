...
burst_types = ['storm'] 
csv_names = ['micro_jpgu_with_dB.csv']
for j in range(len(burst_types)):
    with open(Parent_directory+ '/solar_burst/Nancay/af_sgepss_analysis_data/' + csv_names[j], 'w') as f:
        ...
        w.writeheader()
        check_date_list = []
        for file in glob.glob('/Volumes/GoogleDrive/マイドライブ/lab/solar_burst/Nancay/plot/cleareventfinaljpgu/'+burst_types[j]+'/*/*.png'):
        ...
        for check_date in check_dates:
            ...
            diff_db, diff_db_min_med, min_db, Frequency_start, Frequency_end, resolution, epoch, freq_start_idx, freq_end_idx, Frequency, Status, date_OBs, diff_move_db= read_data(Parent_directory, file_name, 3, 80, 30)
            target_pics = glob.glob('/Volumes/GoogleDrive/マイドライブ/lab/solar_burst/Nancay/plot/cleareventfinaljpgu/'+burst_types[j]+'/'+yyyy+'/'+check_date+'*.png')
