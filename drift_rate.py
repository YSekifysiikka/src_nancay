...
def getNearestValue(list, num):
    ...
    return list[idx]
date_in=[20070101,20090101]
start_day,end_day=date_in
sdate=pd.to_datetime(start_day,format='%Y%m%d')
edate=pd.to_datetime(end_day,format='%Y%m%d')
DATE=sdate

with open(Parent_directory+ '/solar_burst/Nancay/af_sgepss_analysis_data/test/new_gain_analysis_'+str(date_in[0])+'_'+str(date_in[1])+'.csv', 'w') as f:
            ...
            for i in status:
                if i < 50:
                    if i > 0:
                        cali_list = []
                        cali_time = []
                        cali_1 = np.where((cdf_file['Status'][0:i + 2] < 99))
                        cali_2 = np.where((cdf_file['Status'][0:i + 2] > 99))
                        if len(cali_1[0]) > 0:
                    ...
                    cali_list = []
                    cali_time = []
                    cali_1 = np.where((cdf_file['Status'][i-41:i+2] < 99))
                    cali_2 = np.where((cdf_file['Status'][i-41:i+2] > 99))
                    if len(cali_1[0]) > 0:
                        for j in range (len(cali_1[0])):
                    ...
                    if RR_sep.shape[1] >= 40:
                        ...
                        Phot_Pcold = hot_power_all_r - cold_power_all_r
                        Phot_Pcold = np.where(Phot_Pcold <= 0, np.nan, Phot_Pcold)
                        delta_f = 30000#bandwidth
                        gain = Phot_Pcold/(kb*delta_f*Thot_Tcold)
                        Y = hot_power_all_r/cold_power_all_r
                        Y = np.where(Y <= 1, np.nan, Y)
                        Trx = (Thot - (Y*Tcold))/(Y-1)
                        x = np.round(np.arange(Frequency[0], Frequency[-1]+resolution, resolution), decimals=3)
                        x = x[::-1]
                        w.writerow({'obs_time': obs_time, 'Frequency': x, 'Right-gain':gain, 'Right-Trx': Trx, 'Right-hot_dB':hot_power_r_30, 'Right-cold_dB': cold_power_r_30})
                ...
                #         axes_3.plot(x, gain, color = 'r', label = 'gain')
                #         print (gain[index_40dB])
                #         axes_3.set_yscale('log')
                ...
                #         axes_3.plot(x, Trx, color = 'r', label = 'Trx')
                #         axes_3.set_yscale('log')
