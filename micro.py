...
nancay_wind_QL_list = sorted(glob.glob(Parent_directory + '/solar_burst/Nancaywind3/' + yyyy + '/' + mm + '/' + selected_date + '*'))

if sunspots_num_check(yyyy, mm, dd) == True:
        ...
        while t == 0:
            ...
            if select == 'exit':
                ...
                if (int(str(select).split('_')[3])/time_band).is_integer() is True:
                    t_1 = int(int(str(select).split('/')[11].split('_')[3])/time_band)
                else:
