                ...
                if i < 50:
                    ...
                    if RR_sep.shape[1] >= 40:
                        if len(cali_time_f ) == 5:
                            ...
                            x = x[::-1]
                            if np.count_nonzero(np.isnan(gain))/len(gain)*100 < 10:
                                w.writerow({'obs_time': obs_time, 'Frequency': x, 'Right-gain':gain, 'Right-Trx': Trx, 'Right-hot_dB':np.median(RR_sep[:, cali_time_f[-1] - 10:cali_time_f[-1] + 1], axis = 1), 'Right-cold_dB': np.median(RR_sep[:, cali_time_f[0]:cali_time_f[0] + 10], axis = 1)})
