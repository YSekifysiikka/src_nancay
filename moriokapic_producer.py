...
def radio_plot(diff_db_plot_sep, x_lims, Frequency_start, Frequency_end, Time_start, Time_end, date_OBs, burst_type, detected_files, save_dir, width, Status_sep, stime):
    ...
    ax4 = plt.subplot(gs[45:85, :])
    ax2 = plt.subplot(gs[95:110, :])
    ax3 = plt.subplot(gs[115:130, :])
    ax1 = plt.subplot(gs[135:150, :])
                ...
                if len(maxid1_list[(maxid1_list >= maxid3) & (maxid1_list <= maxid3 + 2)]) >= 0:
                    for maxid1 in maxid1_list[(maxid1_list >= maxid3) & (maxid1_list <= maxid3 + 2)]:
                        if maxid1 >= maxid2 + 1:
                            ...
                            burst_time_list.append(dt.datetime(bstime[0], bstime[1], bstime[2], bstime[3], bstime[4], bstime[5], bstime[6]))
...
import pandas as pd
import csv
date_in=[20130401,20130401]
start_day,end_day=date_in
sdate=pd.to_datetime(start_day,format='%Y%m%d')
edate=pd.to_datetime(end_day,format='%Y%m%d')

# with open(Parent_directory+ '/solar_burst/Nancay/af_sgepss_analysis_data/test/morioka_reproduce_'+str(date_in[0])+'_'+str(date_in[1])+'.csv', 'w') as f:
...
#     file_name = file[:-1]
        ...
        for t in range (math.floor(((diff_move_db.shape[1]-time_co)/time_band) + 1)):
            ...
            try:
                dB_list, burst_time_list, dB_BG_40 = radio_plot(diff_db_plot_sep, x_lims, Frequency_start, Frequency_end, Time_start, Time_end, date_OBs[:8], burst_type, detected_files, save_dir, width, Status_sep, time - time_band - time_co)
    ...
    plt.show()
    plt.close()
    DATE+=pd.to_timedelta(30,unit='day')
