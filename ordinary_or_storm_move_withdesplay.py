...
def wind_geotail_nancay_plot_classification(date, DATE, count, check_dir):
    ...
    if len(files) > 0:
        ...
        for file in files:
            ...
            image_nancay = cv2.resize(img_nancay_1, dsize=None, fx=0.15, fy=0.15)
            ...
            image_nancay_QL = cv2.resize(img_nancay_QL_1, dsize=None, fx=1.05, fy=1.05)
            ...
            while check_num == 0:
                ...
                if choice_result == '1':
                    ...
                    if nancay_QL_list.index(plot_nancay_QL) - 1 >= 0:
                        ...
                        img_nancay_QL = cv2.imread(plot_nancay_QL, cv2.IMREAD_COLOR)
                        img_nancay_QL_1 = img_nancay_QL[0:600, 120:1000]
                        image_nancay_QL = cv2.resize(img_nancay_QL_1, dsize=None, fx=1.1, fy=1.1)
                        cv2.namedWindow(WINDOW_NAME_3, cv2.WINDOW_AUTOSIZE)
                        cv2.imshow(WINDOW_NAME_3, image_nancay_QL)
                        cv2.moveWindow(WINDOW_NAME_3, 3655, 560)
                        cv2.waitKey(1)
