...
def wind_geotail_nancay_plot_classification(date, DATE, count, check_dir):
    ...
    if len(files) > 0:
        ...
        for file in files:
            nancay_wind_QL_list = sorted(glob.glob('/Volumes/GoogleDrive/マイドライブ/lab/solar_burst/Nancaywind2/' + yyyy + '/' + mm + '/' + str_date + '*'))
            ...
            if len(Nancay_wind_QL) == 0:
                ...
                if len(Nancay_wind_QL) == 1:
                    ...
                    plot_nancay_wind_QL = glob.glob('/Volumes/GoogleDrive/マイドライブ/lab/solar_burst/Nancaywind2/' + yyyy + '/' + mm + '/' + str_date +  '_*' + str(min(stime_list)) +'_*')[0]
