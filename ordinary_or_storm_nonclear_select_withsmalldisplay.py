...
def choice():
    i = 0
    while i < 1:
        choice = input("Please respond with 'flare_associated_ordinary', 'maybe_ordinary', 'ordinary', 'storm', 'marginal', 'pass', '1', '2', '3', '4', '5', or '6' [f/y/o/s/m/p/1/2/3/4/5/6]: ").lower()
        if choice in ['s']:
            i += 1
            return 'storm'
        elif choice in ['m']:
...
def flare_check(yyyy, dd, mm):
    if csv_input_final['peak'].str.contains(yyyy + '/'+mm+'/'+dd).sum() > 0:
        ...
        plt.savefig(Parent_directory + '/hinode_catalog/flare_plot/' + date1.strftime("%Y%m%d"))
        plt.close()
    return
...
def wind_geotail_nancay_plot_classification(date, DATE, count, check_dir):
    ...
    if len(files) > 0:
        ...
        for file in files:
            ...
            image_nancay = cv2.resize(img_nancay_1, dsize=None, fx=0.9*factor, fy=0.9*factor)
            ...
            axes_.axis("off")
            time_file = Parent_directory + '/solar_burst/dust/aaa.png'
            ...
            if len(sdo_0211_pic) == 0:
                ...
                if len(sdo_0211_pic) == 1:
                    ...
                    plot_sdo_0211 = glob.glob(Parent_directory + '/solar_pic/sdo_0211/' + yyyy + '/' + mm + '/' + str_date +  '_*' + str(min(stime_list)) +'_*')[0]
    ...
    #     img_HMIB = cv2.imread(glob.glob(Parent_directory + "/solar_pic/sdo_HMIB/" + yyyy + "/" + mm+ "/" + yyyy + mm + dd + "*.jpg")[0], cv2.IMREAD_COLOR)
