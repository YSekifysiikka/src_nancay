...
def radio_plot(data_1, receiver_1, data_2, receiver_2, diff_db_plot_sep, x_lims, Frequency_start, Frequency_end, Time_start, Time_end, date_OBs):
    ...
    ax.imshow(diff_db_plot_sep, extent = [x_lims[0], x_lims[1],  y_lims[0], y_lims[1]], 
              aspect='auto',cmap='jet',vmin=0,vmax=8)
    ax.xaxis_date()
    ...
    if not os.path.isdir(Parent_directory + '/solar_burst/Nancaywind/'+year + '/' + month):
        os.makedirs(Parent_directory + '/solar_burst/Nancaywind/'+year + '/' + month)
    filename = Parent_directory + '/solar_burst/Nancaywind/'+year + '/' + month + '/'+year+month+day+'_'+Time_start[0:2]+Time_start[3:5]+Time_start[6:8]+'_'+Time_end[0:2]+Time_end[3:5]+Time_end[6:8]+ '.png'
...
def read_data_LL_RR(Parent_directory, file_name, move_ave, Freq_start, Freq_end):
    ...
    return LL_min, RR_min, diff_db_min_med, min_db, Frequency_start, Frequency_end, resolution, epoch, freq_start_idx, freq_end_idx, Frequency, Status, date_OBs
...
def separated_data_LL_RR(LL, RR, diff_db_min_med, epoch, time_co, time_band, t):
