...
# -*- coding: utf-8 -*-
"""
Created on Thu Mar 25 15:40:19 2021
    ...
    for file in [freq_drift_each_active_list, freq_drift_day_list]:
        if len(file) == 2:
            if file == freq_drift_each_active_list:
                ...
                text = '\nObserved more than ' +  str(average_threshold) + ' events a day'
            freq_drift_micro = file[0]
            freq_drift_ordinary = file[1]
            ...
            for i in range(len(y_hist)):
                ...
                plt.title('Frequency drift rates @ ' + str(freq_check) + '[MH/z]' + text)
...
def start_end_duration_hist_analysis():
...
# ###############################################
#     start = datetime.datetime(x_list[0].year, 1, 1)
#     end = datetime.datetime(x_list[-1].year, 12, 31)
#     while start < end:
#         fig, ax = plt.subplots(figsize=(14, 6))
#         ax.scatter(x_list, y_list)
...
#         plt.tick_params(labelsize=20)
#         plt.ylabel('Occurence Number',fontsize=25)
#         plt.xlim(start, start + relativedelta.relativedelta(months=move_plot) - relativedelta.relativedelta(days=1))

#         # plt.show()
#         plt.close()
#         start += relativedelta.relativedelta(months=move_plot)
...
###############################################
            # print (i)
            print (start_time[i], end_time[i], len(freq_start[list_start_num[i]:list_end_num[i] + 1]))
...
###############################################
start_end_duration_hist_analysis()
###############################################