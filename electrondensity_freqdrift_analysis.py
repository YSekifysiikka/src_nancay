...
with open(Parent_directory+ '/solar_burst/Nancay/af_sgepss_analysis_data/electrondensity_anaysis.csv', 'w') as f:
    w = csv.DictWriter(f, fieldnames=["obs_time", "burst_type", "dfdt_list", "factor_list", "residual_list", "velocity_list"])
    w.writeheader()
    ...
    for i in range(len(obs_time_ordinary)):
        ...
        "obs_time", "burst_type", "dfdt_list", "factor_list", "residual_list"
        w.writerow({'obs_time':obs_time, 'burst_type':burst_type, 'dfdt_list':dfdt_list,'factor_list':factor_list_all, 'residual_list':residual_list_all, 'velocity_list':velocity_list})
