...
for i in range (len(flare_csv['peak'])):
    ...
    if int(yyyy) < 2004:
        ...
        if ((peak_time >= datetime.datetime(2004,9,3)) & (peak_time <= datetime.datetime(2004,9,12))):
...
for i in range(len(csv_input)):
    ...
    if (BG_obs_time_event >= datetime.datetime(2004, 9, 3)) & (BG_obs_time_event <= datetime.datetime(2004, 9, 12)):
...
for flare_time in flare_times:
    ...
    if sunspot_num_list[sunspot_idx] >= 0:
...
for flare_day in flare_days:
    ...
    file_names = []
    files = glob.glob('/Volumes/GoogleDrive/マイドライブ/lab/solar_burst/Nancay/plot/cnn_used_data/200409_micro_events/'+flare_day+'_*compare.png')
