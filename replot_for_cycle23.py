...
def read_data(Parent_directory, file_name, move_ave, Freq_start, Freq_end):
    ...
    return diff_db, diff_db_min_med, min_db, Frequency_start, Frequency_end, resolution, epoch, freq_start_idx, freq_end_idx, Frequency, Status, date_OBs, diff_move_db
...
def separated_data(diff_db, diff_db_min_med, epoch, time_co, time_band, t, diff_move_db):
    ...
    return diff_db_plot_sep, diff_db_sep, x_lims, time, Time_start, Time_end, t, diff_move_db_sep
...
import shutil
def cnn_prepare(arr_5, event_start, event_end, freq_start, freq_end, event_time_gap, freq_gap, vmin_1, vmax_1, date_OBs, Time_start, Time_end, color_setting, image_size, img_rows, img_cols, model, save_place, Frequency, x_lims, burst_types, start_frequency):
    year = date_OBs[0:4]
    month = date_OBs[4:6]
    day = date_OBs[6:8]
    ...
    if start_frequency == 80:
    
        if not os.path.isdir('/Volumes/GoogleDrive/マイドライブ/lab/solar_burst/Nancay/plot/train_cycle23/80/'+burst_types):
            os.makedirs('/Volumes/GoogleDrive/マイドライブ/lab/solar_burst/Nancay/plot/train_cycle23/80/'+burst_types)
        filename = '/Volumes/GoogleDrive/マイドライブ/lab/solar_burst/Nancay/plot/train_cycle23/80/'+burst_types+'/'+year+month+day+'_'+Time_start[0:2]+Time_start[3:5]+Time_start[6:8]+'_'+Time_end[0:2]+Time_end[3:5]+Time_end[6:8]  + '_' + str(time - time_band - time_co) + '_' + str(time) + '_' +event_start+'_'+event_end+'_'+freq_start+'_'+freq_end +'compare.png'
        if os.path.isfile(filename):
          os.remove(filename)
        plt.savefig(filename, bbox_inches=extent)
        # plt.show()
        plt.close()
    elif start_frequency == 69.85:
    
        if not os.path.isdir('/Volumes/GoogleDrive/マイドライブ/lab/solar_burst/Nancay/plot/train_cycle23/70/'+burst_types):
            os.makedirs('/Volumes/GoogleDrive/マイドライブ/lab/solar_burst/Nancay/plot/train_cycle23/70/'+burst_types)
        filename = '/Volumes/GoogleDrive/マイドライブ/lab/solar_burst/Nancay/plot/train_cycle23/70/'+burst_types+'/'+year+month+day+'_'+Time_start[0:2]+Time_start[3:5]+Time_start[6:8]+'_'+Time_end[0:2]+Time_end[3:5]+Time_end[6:8]  + '_' + str(time - time_band - time_co) + '_' + str(time) + '_' +event_start+'_'+event_end+'_'+freq_start+'_'+freq_end +'compare.png'
        if os.path.isfile(filename):
          os.remove(filename)
        plt.savefig(filename, bbox_inches=extent)
        # plt.show()
    ...
    # if predict[0][0] >= 0.5:
    # save_directory = Parent_directory + '/solar_burst/Nancay/plot/'+ save_place +'/'+burst_types
...
# /Volumes/GoogleDrive/マイドライブ/lab/solar_burst/Nancay/plot/test/noise/20130123_154846_155526_27880_28280_88_94_77.2_37.475compare.png
burst_types = ['flare'] 
csv_names = ['replot_for_cycle23.csv']
for j in range(len(burst_types)):
    # with open(Parent_directory+ '/solar_burst/Nancay/af_sgepss_analysis_data/' + csv_names[j], 'w') as f:
    #     w = csv.DictWriter(f, fieldnames=["event_date", "event_hour", "event_minite", "velocity", "residual", "event_start", "event_end", "freq_start", "freq_end", "factor", "peak_time_list", "peak_freq_list", "peak_dB_40MHz"])
    #     w.writeheader()
    check_date_list = []
    for file in glob.glob('/Volumes/GoogleDrive/マイドライブ/lab/solar_burst/Nancay/plot/train_cycle23/80/flare/*.png'):
    ...
    for check_date in check_dates:
        ...
        for start_frequency in [69.85]:
            print (start_frequency)

            diff_db, diff_db_min_med, min_db, Frequency_start, Frequency_end, resolution, epoch, freq_start_idx, freq_end_idx, Frequency, Status, date_OBs, diff_move_db= read_data(Parent_directory, file_name, 3, start_frequency, 30)
            # Frequency_40_index = np.where(Frequency == getNearestValue(Frequency,check_freq))[0][0]
            # print(Frequency_40_index)
            target_pics = glob.glob('/Volumes/GoogleDrive/マイドライブ/lab/solar_burst/Nancay/plot/train_cycle23/80/flare/'+check_date+'*.png')
            for target_pic in target_pics:
                if (int(target_pic.split('/')[11].split('_')[3])/time_band).is_integer() is True:
                    t = int(int(target_pic.split('/')[11].split('_')[3])/time_band)
                else:
                    t = math.floor((diff_db_min_med.shape[1]-time_co)/time_band)
                # for t in range (math.floor(((diff_db_min_med.shape[1]-time_co)/time_band) + 1)):
                diff_db_plot_sep, diff_db_sep, x_lims, time, Time_start, Time_end, t_1, diff_move_db_sep = separated_data(diff_db, diff_db_min_med, epoch, time_co, time_band, t, diff_move_db)
                ...
                if len(arr_5_list) == 0:
                    ...
                    for i in range(len(arr_5_list)):
                        if event_start_list[i] == target_pic.split('/')[11].split('_')[5] and event_end_list[i] == target_pic.split('/')[11].split('_')[6]:
                            if freq_end_list[i] == target_pic.split('/')[11].split('_')[8].split('c')[0]:
                                print ('yes')
                                # print ('a')
                                cnn_prepare(arr_5_list[i], event_start_list[i], event_end_list[i], freq_start_list[i], freq_end_list[i], event_time_gap_list[i], freq_gap_list[i], vmin_1_list[i], vmax_1_list[i], date_OBs, Time_start, Time_end, color_setting, image_size, img_rows, img_cols, cnn_model, save_place, Frequency, x_lims, burst_types[j], start_frequency)
