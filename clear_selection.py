...
def wind_geotail_flare(yyyy, mm, dd, WINDOW_NAME_0, WINDOW_NAME_1, WINDOW_NAME_4):        
    if os.path.isfile("/Volumes/GoogleDrive/マイドライブ/lab/solar_burst/wind/plot_QL/" + yyyy + "/wav_summary_" + yyyy + mm + dd + ".png") == True:
        ...
        image_wind = cv2.resize(img_wind_1, dsize=None, fx=1.24, fy=1.24)
    ...
    if os.path.isfile('/Volumes/GoogleDrive/マイドライブ/lab/solar_burst/geotail/plot_QL/' + yyyy + '/' +yyyy[2:4] + mm + dd +'00.gif') == True:
        ...
        image_geotail = cv2.resize(img_geotail_1, dsize=None, fx=0.9, fy=0.9)
    ...
    if len(glob.glob('/Volumes/GoogleDrive/マイドライブ/lab/hinode_catalog/flare_plot/' + yyyy+mm+dd +'.png')) == 1:
        ...
        image_flare = cv2.resize(img_flare_1, dsize=None, fx=1.455, fy=1.455)
        cv2.namedWindow(WINDOW_NAME_4, cv2.WINDOW_AUTOSIZE)
        cv2.imshow(WINDOW_NAME_4, image_flare)
...
def window_close(file, files, WINDOW_NAME_2):
    if file == files[-1]:
        cv2.waitKey(1)
...
def wind_geotail_nancay_plot_classification(date, DATE, count, check_dir):
    ...
    files = sorted(glob.glob('/Volumes/GoogleDrive/マイドライブ/lab/solar_burst/Nancay/plot/'+file_name+'/' + check_dir + '/'+yyyy+'/'+yyyy+mm+dd+'_*.png'))
    if len(files) > 0:
        ...
        for file in files:
            ...
            image_nancay = cv2.resize(img_nancay_1, dsize=None, fx=0.11, fy=0.11)
            cv2.namedWindow(WINDOW_NAME_2 + str_start_time, cv2.WINDOW_AUTOSIZE)
            cv2.imshow(WINDOW_NAME_2 + str_start_time, image_nancay)
            cv2.moveWindow(WINDOW_NAME_2 + str_start_time, 1450, 1212)
            ...
            image_time = cv2.resize(time_data, dsize=None, fx=1, fy=1)
            cv2.namedWindow('Obs_time', cv2.WINDOW_AUTOSIZE)
            cv2.imshow('Obs_time', image_time)
            cv2.moveWindow('Obs_time', 1450, 1610)
            ...
            cv2.waitKey(1)
            cv2.destroyWindow(WINDOW_NAME_3 + '2')
            cv2.waitKey(1)
            img_nancay_QL = cv2.imread(plot_nancay_QL, cv2.IMREAD_COLOR)
            img_nancay_QL_1 = img_nancay_QL[0:600, 140:956]
            image_nancay_QL = cv2.resize(img_nancay_QL_1, dsize=None, fx=1.16, fy=1.16)
            # img_nancay_QL_1 = img_nancay_QL[0:400, 120:1800]
            # image_nancay_QL = cv2.resize(img_nancay_QL_1, dsize=None, fx=0.72, fy=0.72)
            cv2.namedWindow(WINDOW_NAME_3 + '2', cv2.WINDOW_AUTOSIZE)
            cv2.imshow(WINDOW_NAME_3 + '2', image_nancay_QL)
            cv2.moveWindow(WINDOW_NAME_3 + '2', 3355, 860)

            if nancay_QL_list.index(plot_nancay_QL) - 1 >= 0:
                cv2.waitKey(1)
                cv2.destroyWindow(WINDOW_NAME_3 + '1')
                cv2.waitKey(1)
                plot_nancay_QL_1 = nancay_QL_list[nancay_QL_list.index(plot_nancay_QL) - 1]
                img_nancay_QL = cv2.imread(plot_nancay_QL_1, cv2.IMREAD_COLOR)
                img_nancay_QL_1 = img_nancay_QL[0:600, 140:956]
                image_nancay_QL = cv2.resize(img_nancay_QL_1, dsize=None, fx=1.16, fy=1.16)
                cv2.namedWindow(WINDOW_NAME_3 + '1', cv2.WINDOW_AUTOSIZE)
                cv2.imshow(WINDOW_NAME_3 + '1', image_nancay_QL)
                cv2.moveWindow(WINDOW_NAME_3 + '1', 2409, 860)
                cv2.waitKey(1)
            else:
            ...
            if len(nancay_QL_list) > nancay_QL_list.index(plot_nancay_QL) + 1:
                cv2.waitKey(1)
                cv2.destroyWindow(WINDOW_NAME_3 + '3')
                cv2.waitKey(1)
                plot_nancay_QL_2 = nancay_QL_list[nancay_QL_list.index(plot_nancay_QL) + 1]
                img_nancay_QL = cv2.imread(plot_nancay_QL_2, cv2.IMREAD_COLOR)
                img_nancay_QL_1 = img_nancay_QL[0:600, 140:956]
                image_nancay_QL = cv2.resize(img_nancay_QL_1, dsize=None, fx=1.16, fy=1.16)
                cv2.namedWindow(WINDOW_NAME_3 + '3', cv2.WINDOW_AUTOSIZE)
                cv2.imshow(WINDOW_NAME_3 + '3', image_nancay_QL)
                cv2.moveWindow(WINDOW_NAME_3 + '3', 4301, 860)
                cv2.waitKey(1)
            else:
            ...
            if len(Nancay_wind_QL) == 0:
                ...
                cv2.waitKey(1)
                cv2.destroyWindow(WINDOW_NAME_6 + '2')
                cv2.waitKey(1)
                img_nancay_wind_QL = cv2.imread(plot_nancay_wind_QL, cv2.IMREAD_COLOR)
                img_nancay_wind_QL_1 = img_nancay_wind_QL[100:800, 105:780]
                image_nancay_wind_QL = cv2.resize(img_nancay_wind_QL_1, dsize=None, fx=1.4, fy=1.3)
                # img_nancay_QL_1 = img_nancay_QL[0:400, 120:1800]
                # image_nancay_QL = cv2.resize(img_nancay_QL_1, dsize=None, fx=0.72, fy=0.72)
                cv2.namedWindow(WINDOW_NAME_6 + '2', cv2.WINDOW_AUTOSIZE)
                cv2.imshow(WINDOW_NAME_6 + '2', image_nancay_wind_QL)
                cv2.moveWindow(WINDOW_NAME_6 + '2', 3355, 0)
    
                if nancay_wind_QL_list.index(plot_nancay_wind_QL) - 1 >= 0:
                    cv2.waitKey(1)
                    cv2.destroyWindow(WINDOW_NAME_6 + '1')
                    cv2.waitKey(1)
                    plot_nancay_wind_QL_1 = nancay_wind_QL_list[nancay_wind_QL_list.index(plot_nancay_wind_QL) - 1]
                    img_nancay_wind_QL = cv2.imread(plot_nancay_wind_QL_1, cv2.IMREAD_COLOR)
                    img_nancay_wind_QL_1 = img_nancay_wind_QL[100:800, 105:780]
                    image_nancay_wind_QL = cv2.resize(img_nancay_wind_QL_1, dsize=None, fx=1.4, fy=1.3)
                    # img_nancay_QL_1 = img_nancay_QL[0:400, 120:1800]
                    # image_nancay_QL = cv2.resize(img_nancay_QL_1, dsize=None, fx=0.72, fy=0.72)
                    cv2.namedWindow(WINDOW_NAME_6 + '1', cv2.WINDOW_AUTOSIZE)
                    cv2.imshow(WINDOW_NAME_6 + '1', image_nancay_wind_QL)
                    cv2.moveWindow(WINDOW_NAME_6 + '1', 2409, 0)
                    cv2.waitKey(1)
                else:
                ...
                if len(nancay_wind_QL_list) > nancay_wind_QL_list.index(plot_nancay_wind_QL) + 1:
                    cv2.waitKey(1)
                    cv2.destroyWindow(WINDOW_NAME_6 + '3')
                    cv2.waitKey(1)
                    plot_nancay_wind_QL_2 = nancay_wind_QL_list[nancay_wind_QL_list.index(plot_nancay_wind_QL) + 1]
                    img_nancay_wind_QL = cv2.imread(plot_nancay_wind_QL_2, cv2.IMREAD_COLOR)
                    img_nancay_wind_QL_1 = img_nancay_wind_QL[100:800, 105:780]
                    image_nancay_wind_QL = cv2.resize(img_nancay_wind_QL_1, dsize=None, fx=1.4, fy=1.3)
                    # img_nancay_QL_1 = img_nancay_QL[0:400, 120:1800]
                    # image_nancay_QL = cv2.resize(img_nancay_QL_1, dsize=None, fx=0.72, fy=0.72)
                    cv2.namedWindow(WINDOW_NAME_6 + '3', cv2.WINDOW_AUTOSIZE)
                    cv2.imshow(WINDOW_NAME_6 + '3', image_nancay_wind_QL)
                    cv2.moveWindow(WINDOW_NAME_6 + '3', 4301, 0)
            ...
            #     image_sdo_0193 = cv2.resize(img_sdo_0193_1, dsize=None, fx=0.9, fy=0.9)
            ...
            #     image_sdo_0211 = cv2.resize(img_sdo_0211_1, dsize=None, fx=0.9, fy=0.9)
            #     cv2.namedWindow(WINDOW_NAME_8+sdo_time_0211, cv2.WINDOW_AUTOSIZE)
            #     cv2.imshow(WINDOW_NAME_8+sdo_time_0211, image_sdo_0211)
            #     cv2.moveWindow(WINDOW_NAME_8+sdo_time_0211, 1910, 1212)
            ...
            while check_num == 0:
                ...
                if choice_result == '1':
                    ...
                    if nancay_QL_list.index(plot_nancay_QL) - 1 >= 0:
                        ...
                        image_nancay_QL = cv2.resize(img_nancay_QL_1, dsize=None, fx=1.8, fy=1.8)
                    ...
                    if nancay_wind_QL_list.index(plot_nancay_wind_QL) - 1 >= 0:
                        ...
                        image_nancay_wind_QL = cv2.resize(img_nancay_wind_QL_1, dsize=None, fx=1.66, fy=1.66)
                        # img_nancay_QL_1 = img_nancay_QL[0:400, 120:1800]
                        # image_nancay_QL = cv2.resize(img_nancay_QL_1, dsize=None, fx=0.72, fy=0.72)
                ...
                #             print ('No data found: SDO 0211')
                    ...
                    if choice_result == check_dir:
                        ...
                        window_close(file, files, WINDOW_NAME_2 + str_start_time)
...
if __name__=='__main__':
    ...
    for year in year_list:
        check_plots = glob.glob('/Volumes/GoogleDrive/マイドライブ/lab/solar_burst/Nancay/plot/'+file_name+'/' + check_dir + '/'+year+'/*.png')
