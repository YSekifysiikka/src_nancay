...
# -*- coding: utf-8 -*-
"""
Created on Wed Sep  2 15:35:42 2020
...
with open(Parent_directory+ '/solar_burst/Nancay/analysis_data/residual_test_shuron' + year + '.csv', 'w') as f:
    ...
    #plot_date_select
    Obs_date = []
    path = Parent_directory + '/solar_burst/Nancay/plot/'+ after_plot +'/flare/simple/*' + '.png'
    ...
    Obs_date_list = np.unique(Obs_date)
    
    #from_20110404_30-80MHz
    Obs_date_final = []
    for x in range(len(Obs_date_list)):
        if int(Obs_date_list[x]) >= start:
            if int(Obs_date_list[x]) <= end:
                Obs_date_final.append(Obs_date_list[x])
    Obs_date_final = sorted(Obs_date_final)
    ...
    #        path = Parent_directory + '/solar_burst/Nancay/plot/'+ after_plot +'/'+ after_after_check +'/' +str(sigma_value)+ '/' + year + '/'+ month +'/'+ Obs_date_final[x] +'*'
        path = Parent_directory + '/solar_burst/Nancay/plot/'+ after_plot +'/flare/simple/'+ Obs_date_final[x] +'*' + '.png'
        File = glob.glob(path, recursive=True)
        File1=len(File)
        # print (File1)
        for cstr in File:
    ...
    ######################################################################################
                                                                    ...
                                                                    #         # gs = gridspec.GridSpec(21, 12)
                                                                            ...
                                                                            # short_start_1 = epoch[short_start + 1]
                                                                            # short_end_1 = epoch[short_end + 1]
                                                                            # start_new = dt.datetime(short_start_1[0], short_start_1[1], short_start_1[2], short_start_1[3], short_start_1[4], short_start_1[5], short_start_1[6])
                                                                            # end_new = dt.datetime(short_end_1[0], short_end_1[1], short_end_1[2], short_end_1[3], short_end_1[4], short_end_1[5], short_end_1[6])
                                                                    ...
                                                                    #         plt.show()
                                                                            ...
                                                                            # arr6 = np.where(try_list > -10, (10 ** (try_list/10)), try_list)[:286]
    ...
    #                                                                                axes_2.plot(yy_1, xx_2, 'k', label = 'freq_drift(linear)')
                                                                            ...
                                                                            if not os.path.isdir(Parent_directory + '/solar_burst/Nancay/plot/'+plot_dic):
                                                                                os.makedirs(Parent_directory + '/solar_burst/Nancay/plot/'+plot_dic)
                                                                            filename = Parent_directory + '/solar_burst/Nancay/plot/'+plot_dic+'/'+year+month+day+'_'+Time_start[0:2]+Time_start[3:5]+Time_start[6:8]+'_'+Time_end[0:2]+Time_end[3:5]+Time_end[6:8]+ '_' + str(time - time_band - time_co) + '_' + str(time) +'_' + event_start+'_'+event_end+'_'+freq_start+'_'+freq_end+ 'peak.png'
                                                                            plt.savefig(filename)
                                                                            # plt.show()
                                                                            plt.close()
