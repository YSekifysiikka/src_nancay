...
def analysis_bursts(DATE, FDATE, csv_input_final, check_burst):
        ...
        try:
            ...
            for j in range(len(csv_input_final)):
                if csv_input_final['event_date'][j] >= sdate and csv_input_final['event_date'][j] <= edate:
                    if csv_input_final['event_number'][j] == check_burst:
...
if __name__=='__main__':
    ...
    # print(date)
    try:
        obs_time_ordinary, freq_drift_ordinary, repeat_num, freq_start_ordinary, freq_end_ordinary, duration_ordinary, obs_time_all_ordinary, event_number_ordinary, event_number_all_ordinary = analysis_bursts(DATE, FDATE, csv_ordinary, check_burst)
    except:
        print('DL error: ',DATE)
    ...
    # print(date)
    try:
        obs_time_micro, freq_drift_micro, repeat_num, freq_start_micro, freq_end_micro, duration_micro, obs_time_all_micro, event_number_micro, event_number_all_micro = analysis_bursts(DATE, FDATE, csv_micro, check_burst)
    except:
        print('DL error: ',DATE)


burst_types = ["Micro-type Ⅲ burst"]
for burst_type in burst_types:
    if burst_type == "Micro-type Ⅲ burst":
        ...
        break
    print (obs_time)
    freq_drift_mean_list = freq_drift
    time_mean_list = obs_time

    analysis_period = [min(time_mean_list)-relativedelta.relativedelta(days=1), max(time_mean_list)+relativedelta.relativedelta(days=1)]
###############################################
    # 連続で観測されたデータの平均

    breakpoint_1 = time_mean_list.index(getNearestValue(time_mean_list, analysis_period[0]))
    breakpoint_2 = time_mean_list.index(getNearestValue(time_mean_list, analysis_period[1]))
    if not time_mean_list.count(getNearestValue(time_mean_list, analysis_period[1])) == 1:
...
###############################################
        #太陽極大期と極小期のセレクト
        if analysis_period[0] <= time_mean_list[i] and analysis_period[1] >= time_mean_list[i]:
    ###############################################
...
###############################################
    #連続で観測されたデータの平均
    period = [min(time_mean_list)-relativedelta.relativedelta(days=1), max(time_mean_list)+relativedelta.relativedelta(days=1)]
