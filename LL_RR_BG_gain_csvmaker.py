...
file_final = '/solar_burst/Nancay/af_sgepss_analysis_data/RR_LL_antenna/RR_LL_all_freq_under25_20070101_20201231.csv'
...
import csv
with open(Parent_directory+ '/solar_burst/Nancay/af_sgepss_analysis_data/RR_LL_gain_BG/under25_RR_LL_gain_BG_analysis_calibrated_gainfixed.csv', 'w') as f:
    w = csv.DictWriter(f, fieldnames=["obs_time", "Frequency", "Right-BG_Calibrated", "Left-BG_Calibrated", "Right-BG", "Left-BG", "Right-gain", "Right-Trx", "Right-hot_dB", "Right-cold_dB", "Left-gain", "Left-Trx", "Left-hot_dB", "Left-cold_dB", "Used_dB_median"])
    w.writeheader()
    for i in range(len(BG_obs_times)):
        if np.abs(BG_obs_times[i] - getNearestValue(gain_obs_times, BG_obs_times[i])) <= datetime.timedelta(seconds=3600):
            gain_idx = np.where(gain_obs_times==getNearestValue(gain_obs_times, BG_obs_times[i]))[0][0]
            cali_BG_list_r = decibel_list_r[i]+(np.log10((np.ones(gain_RR.shape[1])*6.6e+19)/gain_RR[gain_idx]) * 10)
            cali_BG_list_l = decibel_list_l[i]+(np.log10((np.ones(gain_RR.shape[1])*6.6e+19)/gain_LL[gain_idx]) * 10)
            w.writerow({'obs_time': BG_obs_times[i], 'Frequency': Frequency[gain_idx], 'Right-BG_Calibrated':cali_BG_list_r, 'Left-BG_Calibrated': cali_BG_list_l, 'Right-BG':decibel_list_r[i], 'Left-BG': decibel_list_l[i], 'Right-gain':gain_RR[gain_idx], 'Right-Trx': Trx_RR[gain_idx], 'Right-hot_dB':hot_dB_RR[gain_idx], 'Right-cold_dB': cold_dB_RR[gain_idx], 'Left-gain':gain_LL[gain_idx], 'Left-Trx': Trx_LL[gain_idx], 'Left-hot_dB':hot_dB_LL[gain_idx], 'Left-cold_dB': cold_dB_LL[gain_idx], 'Used_dB_median':(np.ones(gain_RR.shape[1])*6.6e+19)})
