...
# -*- coding: utf-8 -*-
"""
Created on Tue Oct  5 15:45:15 2021
...
file2 = '/Volumes/GoogleDrive/マイドライブ/lab/solar_burst/Nancay/af_sgepss_analysis_data/RR_LL_gain/new_ver2_gain_analysis_20070101_20201231.csv'
...
with open(Parent_directory+ '/solar_burst/Nancay/af_sgepss_analysis_data/test/RR_LL_30dB_40dB_gain_analysis.csv', 'w') as f:
    ...
    for file in file_list:
        ...
        for i in range(len(csv_input)):
            ...
            Gain_list_r = np.array([float(k) for k in csv_input['Right-gain'][i][1:-1].replace('\n', '').split(' ') if k != ''])
            gain_r = Gain_list_r[[freq_idx_40, freq_idx_37_5, freq_idx_35, freq_idx_32_5, freq_idx_30]]
    
            Trx_list_r = np.array([float(k) for k in csv_input['Right-Trx'][i][1:-1].replace('\n', '').split(' ') if k != ''])
            hot_dB_list_r = np.array([float(k) for k in csv_input['Right-hot_dB'][i][1:-1].replace('\n', '').split(' ') if k != ''])
            cold_dB_list_r = np.array([float(k) for k in csv_input['Right-cold_dB'][i][1:-1].replace('\n', '').split(' ') if k != ''])
            Trx_r = Trx_list_r[[freq_idx_40, freq_idx_37_5, freq_idx_35, freq_idx_32_5, freq_idx_30]]
            hot_dB_r = hot_dB_list_r[[freq_idx_40, freq_idx_37_5, freq_idx_35, freq_idx_32_5, freq_idx_30]]
            cold_dB_r = cold_dB_list_r[[freq_idx_40, freq_idx_37_5, freq_idx_35, freq_idx_32_5, freq_idx_30]]

            Gain_list_l = np.array([float(k) for k in csv_input['Left-gain'][i][1:-1].replace('\n', '').split(' ') if k != ''])
            gain_l = Gain_list_l[[freq_idx_40, freq_idx_37_5, freq_idx_35, freq_idx_32_5, freq_idx_30]]
    
            Trx_list_l = np.array([float(k) for k in csv_input['Left-Trx'][i][1:-1].replace('\n', '').split(' ') if k != ''])
            hot_dB_list_l = np.array([float(k) for k in csv_input['Left-hot_dB'][i][1:-1].replace('\n', '').split(' ') if k != ''])
            cold_dB_list_l = np.array([float(k) for k in csv_input['Left-cold_dB'][i][1:-1].replace('\n', '').split(' ') if k != ''])
            Trx_l = Trx_list_l[[freq_idx_40, freq_idx_37_5, freq_idx_35, freq_idx_32_5, freq_idx_30]]
            hot_dB_l = hot_dB_list_l[[freq_idx_40, freq_idx_37_5, freq_idx_35, freq_idx_32_5, freq_idx_30]]
            cold_dB_l = cold_dB_list_l[[freq_idx_40, freq_idx_37_5, freq_idx_35, freq_idx_32_5, freq_idx_30]]

            # # print ('a')
            # if not len(Gain_list) == len(Frequency):
            #     print ('c')
            #     sys.exit()
            w.writerow({'obs_time': obs_time, 'Frequency': Frequency[[freq_idx_40, freq_idx_37_5, freq_idx_35, freq_idx_32_5, freq_idx_30]], 'Right-gain':gain_r, 'Right-Trx': Trx_r, 'Right-hot_dB':hot_dB_r, 'Right-cold_dB': cold_dB_r, 'Left-gain':gain_l, 'Left-Trx': Trx_l, 'Left-hot_dB':hot_dB_l, 'Left-cold_dB': cold_dB_l})
