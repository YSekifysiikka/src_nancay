...
average_threshold = 1
error_threshold = 3
solar_maximum = [datetime.datetime(2012, 1, 1), datetime.datetime(2014, 12, 31)]
solar_minimum = [datetime.datetime(2017, 1, 1), datetime.datetime(2020, 12, 31)]
...
csv_flare_list = pd.read_csv(filepath_or_buffer= Parent_directory + file_final, sep=",")
...
def analysis_bursts(DATE, FDATE, csv_input_final):
        ...
        try:
            ...
            for j in range(len(csv_input_final)):
                if csv_input_final['event_date'][j] >= sdate and csv_input_final['event_date'][j] <= edate:
                    ...
                    event_number_all.append(csv_input_final['event_number'][j])
                    if csv_input_final[["freq_start"][0]][j] >= freq_check:
                        if csv_input_final[["freq_end"][0]][j] <= freq_check:
                            ...
                            for z in range(separate_num):
                                ...
                                event_number.append(csv_input_final['event_number'][j])
    ...
    return obs_time, freq_drift_final, repeat_num, start_check_list, end_check_list, duration_list, obs_time_all, event_number, event_number_all
...
date_in=[20120101, 20191231]

if __name__=='__main__':
    start_day, end_day=date_in
    DATE=pd.to_datetime(start_day,format='%Y%m%d')
    FDATE = pd.to_datetime(end_day,format='%Y%m%d')
    file_final = "/solar_burst/Nancay/af_sgepss_analysis_data/ordinary_burst_jpgu_final.csv"
    csv_ordinary = pd.read_csv(filepath_or_buffer= Parent_directory + file_final, sep=",")
    
    # DATE=sdate
    # date=DATE.strftime(format='%Y%m%d')
    # print(date)
    try:
        obs_time_ordinary, freq_drift_ordinary, repeat_num, freq_start_ordinary, freq_end_ordinary, duration_ordinary, obs_time_all_ordinary, event_number_ordinary, event_number_all_ordinary = analysis_bursts(DATE, FDATE, csv_ordinary)
    except:
        print('DL error: ',DATE)
    ...
    DATE=pd.to_datetime(start_day,format='%Y%m%d')
    FDATE = pd.to_datetime(end_day,format='%Y%m%d')
    file_final = "/solar_burst/Nancay/af_sgepss_analysis_data/micro_burst_jpgu_final.csv"
    csv_micro = pd.read_csv(filepath_or_buffer= Parent_directory + file_final, sep=",")
    
    # DATE=sdate
    # date=DATE.strftime(format='%Y%m%d')
    # print(date)
    try:
        obs_time_micro, freq_drift_micro, repeat_num, freq_start_micro, freq_end_micro, duration_micro, obs_time_all_micro, event_number_micro, event_number_all_micro = analysis_bursts(DATE, FDATE, csv_micro)
    except:
        print('DL error: ',DATE)
...
for burst_type in burst_types:
    ...
    freq_drift_std_list = []
    time_mean_list = []
    for i in range(max(event_number)):
        freq_drift_list = []
        time_list = []
        for j in range(len(event_number)):
            if event_number[j] == i:
                freq_drift_list.append(freq_drift[j])
                time_list.append(obs_time[j])

        if len(time_list) == 0:
            pass
        elif len(time_list) >= average_threshold:
            time_mean = time_list[0] + (time_list[-1] - time_list[0])/2
            freq_drift_mean_list.append(np.mean(freq_drift_list))
            time_mean_list.append(time_mean)
            if len(time_list) >= error_threshold:
                freq_drift_std_list.append(np.std(freq_drift_list))
            else:
                freq_drift_std_list.append(1000)
        else:
            # print ('Lower than event number threshold')
            pass

###############################################
    # 連続で観測されたデータの平均
    break_point = time_mean_list.index(getNearestValue(time_mean_list, datetime.datetime(2017, 1, 1, 0, 0)))
    for period in analysis_period:
        breakpoint_1 = time_mean_list.index(getNearestValue(time_mean_list, period[0]))
        breakpoint_2 = time_mean_list.index(getNearestValue(time_mean_list, period[1]))
        if not time_mean_list.count(getNearestValue(time_mean_list, period[1])) == 1:
            print ('breakpoint have to be changed')
            sys.exit()
            
        fig, ax = plt.subplots(figsize=(14, 6))
        for i in range(len(time_mean_list)):
###############################################
            #太陽極大期と極小期のセレクト
            if period[0] <= time_mean_list[i] and period[1] >= time_mean_list[i]:
        ###############################################
                if not freq_drift_std_list[i] == 1000:
                    ax.errorbar(time_mean_list[i], freq_drift_mean_list[i], yerr=freq_drift_std_list[i], color = 'b', marker = 's')
                else:
                    ax.scatter(time_mean_list[i], freq_drift_mean_list[i], color = 'b')
    ...
    ###############################################
        ...
        for i in range(60):
            ...
            for j in range(len(time_mean_list)):
                if time_mean_list[j] >= datetime.datetime(time_mean_list[0].year, 1, 1)+relativedelta.relativedelta(months=analysis_move_ave*i) and time_mean_list[j] <= datetime.datetime(time_mean_list[0].year, 1, 1)+ relativedelta.relativedelta(months=analysis_move_ave*(i+1)) - relativedelta.relativedelta(days=1):
        ...
        plt.show()
        plt.close()
        freq_drift_each_active_list.append(freq_drift_mean_list[breakpoint_1:breakpoint_2+1])
...
#周波数ドリフト率
    ...
    for i in range(len(obs_time)):
        if i == 0:
            ...
            if not obs_time[0].strftime(format='%Y%m%d') == obs_time[1].strftime(format='%Y%m%d'):
                end_time.append(obs_time[i])
                list_end_num.append(i)
        elif i == len(obs_time) - 1:
            end_time.append(obs_time[i])
            list_end_num.append(i)
            if not obs_time[-1].strftime(format='%Y%m%d') == obs_time[-2].strftime(format='%Y%m%d'):
                start_time.append(obs_time[i])
                list_start_num.append(i)
        else:
            if not (obs_time[i].strftime(format='%Y%m%d') == obs_time[i-1].strftime(format='%Y%m%d')) and not (obs_time[i].strftime(format='%Y%m%d') == obs_time[i+1].strftime(format='%Y%m%d')):
                start_time.append(obs_time[i])
                list_start_num.append(i)
                end_time.append(obs_time[i])
                list_end_num.append(i)
            else:
                if not (obs_time[i].strftime(format='%Y%m%d') == obs_time[i-1].strftime(format='%Y%m%d')):
